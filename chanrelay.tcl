...
proc ::crelay::init {args} {
    ...
    if { $me(transmit) == "y" } {
        ...
        if { $me(oper) == "y" } {
            ...
            bind pub -|o "@voice" [namespace current]::trans:ovoice
            bind pub -|o "@hop" [namespace current]::trans:ohop
            bind pub -|o "@op" [namespace current]::trans:oop
        }
    }
    ...
    if { $me(receive) =="y" } {
        ...
        bind bot - ">list" [namespace current]::recv:list
        bind bot - ">users" [namespace current]::recv:users
    }
    ...
    bind msg -|o "rc.debug" [namespace current]::set:debug
...
namespace eval crelay {
    ...
    proc set:debug { nick host handle arg} {
        set arg [string tolower $arg]
        if { $arg == "yes" } {
            set [namespace current]::debug 1
            puthelp "NOTICE $nick :Debug mode is now enabled"
        } elseif { $arg == "no" } {
            set [namespace current]::debug 0
    ...
    proc trans:ovoice {nick uhost handle chan text} {
        ...
        [namespace current]::trans:omode $nick $uhost $handle $chan "[join [lindex [split $text] 0]] $mode $vict"
    }
    ...
    proc trans:ohop {nick uhost handle chan text} {
        ...
        set vict [string range [join [lrange [split $text] 1 end]] 1 end]
        [namespace current]::trans:omode $nick $uhost $handle $chan "@mode [join [lindex [split $text] 0]] $mode $vict"
    }
